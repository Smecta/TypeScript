...
7. 对象
   ...
   3. 对象的类型注解
      ...
      2. 对象方法的类型注解
         ...
         2. 返回值
   4. 接口的使用
      1. 为对象的类型注解命名，并为你的代码建立联系来约束对象的结构
      2. ``` interface 名称{类型注解}```
      3. 接口名称约定以I来开头，使用接口来作为对象的类型注解
   5. 对象的取值和存值
      1. 获取对象中的属性 称为 访问属性
      2. 获取对象中的方法并调用 称为 调用方法
      3. 使用点语法（.)
   6. 内置对象
      1. 数组就是内置对象
      2. 掌握查文档就是一种方式 mdn w3chool
   7. 数组对象
      1. 数组的常用操作
         1. 添加
         2. 删除
      ...
      2. 属性 
         1. length属性 数组长度
      3. 方法
         1. push() 方法将一个或者多个元素添加到数组的末尾，并返回该数组的新长度
         2. forEach()方法 遍历数组 参数是一个函数，这种函数也称为**回调函数**
            1. 回调函数 的两个参数 第一个参数 表示数组中的每个元素 第二参数 表示索引即下标
            2. 此时的回调函数是作为方法的实参传入，不应该指定类型注解
            3. 回调函数的参数可以用任意名称，并且如果没有用到，可以省略
         3. some()方法 遍历数组 查找是否有一个满足条件的元素 如果有则停止循环
            1. 根据回调函数的返回值，决定是否停止循环，如果返回true 就停止，否则继续循环
            2. some方法的返回值：布尔值 如果找到满足条件的元素，结果为true 否则为false
            * 如果查找是否包含满足条件的元素时，使用**some**
            * 对数组中每个元素都进行相同的处理时，使用**forEach**(因为forEach 会遍历整个数组)
8. TS的类型推论
   1. TS会帮助我们提供类型
      1. 声明变量并初始化时
         1. 如果先声明变量后赋值，此时应该在声明变量时指定类型注解
      2. 决定函数返回值时
   2. 这两种情况下类型注解可以省略不写，能省略就省略（提高开发效率）
